...
begin(uint8_t I2C_ADDR, int DRDY_pin, TwoWire &wirePort, uint8_t gain, uint8_t digFilt)
{
  ...
  uint8_t hallStatus = setHallConf(0x0C);
  ...
  uint8_t status7 = setTemperatureOverSampling(0);
  uint8_t status8 = setExtTrig(0);
  
  //dig filt =4,3,2 OK after adding 3*67us to the timin
  return status1 | hallStatus | status2 | status3 | status4 | status5 | status6 | status7 | status8;
}
...
{
  if (hasError(cache_fill())) { return STATUS_ERROR; }
...
{
  if (hasError(cache_fill())) { lastReadError = 6; return STATUS_ERROR; }
...
readMeasurement(uint8_t zyxt_flags, txyzRaw& txyz_result)
{
  ...
  if (hasError(buffer[0])) { lastReadError = 5; return buffer[0]; }
  uint8_t i = 1;
...
readRegister(uint8_t address, uint16_t& data)
{
  ...
  if (hasError(status)) { return STATUS_ERROR; }
...
{
  data = txyz{NAN, NAN, NAN, NAN};
  if (hasError(cache_fill())) { return STATUS_ERROR; }
  uint8_t status1 = startMeasurement(X_FLAG | Y_FLAG | Z_FLAG | T_FLAG);
  if (hasError(status1)) { return STATUS_ERROR; }
  ...
  if (DRDY_pin >= 0){
    ...
    while(!digitalRead(DRDY_pin)){
      if (static_cast<uint32_t>(millis() - started) >= 250) { return STATUS_ERROR; }
    }
  } else {
  ...
  if (hasError(status2)) { return STATUS_ERROR; }
...
readBurstData(MLX90393::txyz& data)
{
  ...
  txyzRaw raw_txyz{};
  uint8_t status2 = readMeasurement(X_FLAG | Y_FLAG | Z_FLAG | T_FLAG, raw_txyz);
  if (hasError(status2)) { return status2; }
  data = convertRaw(raw_txyz);
  return status2;
}
