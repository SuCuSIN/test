...
class AnySkinMonitor:
    ...
    def calibrate(self, samples: int | None = None, cancel_requested=None,
                  preserve_contact_baseline=False, contact_drift_limit=None,
                  contact_stability_limit=None) -> bool:
        if not self.enabled:
        ...
        baselines = []
        model_baselines = []
        self.last_rezero_rejection = None
        started = time.monotonic()
        # Both serial readers are already running; collect the same fresh window.
        start_counts = [getattr(stream, "sample_cnt", 0) for stream in self.streams]
        deadline = started + max(3.0, samples * 0.05)
        print(f"AnySkin zero calibration: waiting for {samples} fresh samples per sensor concurrently.")
        while any(getattr(stream, "sample_cnt", 0) < count + samples
                  for stream, count in zip(self.streams, start_counts)):
            if cancel_requested is not None and cancel_requested():
                return False
            if time.monotonic() >= deadline:
                raise RuntimeError("AnySkin calibration timed out waiting for fresh samples")
            time.sleep(0.01)
        for sensor_index, stream in enumerate(self.streams, start=1):
            ...
            if readings.ndim != 2 or readings.shape[0] < samples:
                raise RuntimeError(f"No AnySkin samples received from sensor {sensor_index}")
            readings = readings[:, 1:]
            if readings.size == 0 or not self._np.isfinite(readings).all():
                raise RuntimeError(f"Invalid AnySkin calibration samples from sensor {sensor_index}")
            baselines.append(self._np.median(readings, axis=0))
            if contact_stability_limit is not None:
                spread = self._np.percentile(readings, 90, axis=0) - self._np.percentile(readings, 10, axis=0)
                if self._np.max(self._np.linalg.norm(spread.reshape(-1, 3), axis=1)) > contact_stability_limit:
                    self.last_rezero_rejection = f'sensor {sensor_index}: unstable open readings'
                    return False
            if contact_drift_limit is not None:
                reference = getattr(self, 'contact_zero_reference', None)
                if reference is None or len(reference) != len(self.streams):
                    self.last_rezero_rejection = 'initial contact zero reference unavailable'
                    return False
                shift = baselines[-1] - self._np.asarray(reference[sensor_index - 1])
                spread = self._np.percentile(readings, 90, axis=0) - self._np.percentile(readings, 10, axis=0)
                if (self._np.max(self._np.linalg.norm(shift.reshape(-1, 3), axis=1)) > contact_drift_limit
                        or self._np.max(self._np.linalg.norm(spread.reshape(-1, 3), axis=1)) > contact_drift_limit):
                    self.last_rezero_rejection = f'sensor {sensor_index}: large drift or unstable open readings'
                    return False
        ...
        with self._sample_lock:
            if cancel_requested is not None and cancel_requested():
                return False
            # Open-pose tare assumes the unloaded signal changed additively.
            # Keep the independently measured empty-close motion envelope unchanged.
            if not preserve_contact_baseline:
                self.baselines = baselines
                self._filtered_magnet_strengths = []
                if contact_drift_limit is None and contact_stability_limit is None:
                    self.contact_zero_reference = [row.copy() for row in baselines]
        ...
        print(
            ...
            f"({sensor_text}); elapsed={time.monotonic() - started:.2f}s. "
            "Keep sensors unloaded while calibrating."
        )
        return True
